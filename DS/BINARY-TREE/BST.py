...
class binary_search_tree:
	...
	def size(self):
		if self.root!=None:
			...
			return 0
	def _size(self,cur_node,cur_size):
		if cur_node==None: return cur_height
		if cur_node.left_child and cur_node.right_child:
                        return 1+self._size(cur_node.left_height,cur_size)+self._size(cur_node.right_height,cur_size)
                if cur_node.left_child:
                        return 1+self._size(cur_node.left_height,cur_size)
                else:
                        return 1+ self._size(cur_node.right_height,cur_size)
